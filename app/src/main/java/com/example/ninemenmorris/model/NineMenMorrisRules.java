package com.example.ninemenmorris.model;
/*
...
public class NineMenMorrisRules implements Parcelable {
	...
	private int bluemarker, redmarker, blueTurns, redTurns;
	...
	protected NineMenMorrisRules(Parcel in) {
		gameplan = in.createIntArray();
		bluemarker = in.readInt();
		redmarker = in.readInt();
		blueTurns = in.readInt();
		redTurns = in.readInt();
		turn = in.readInt();
	}

	public static final Creator<NineMenMorrisRules> CREATOR = new Creator<NineMenMorrisRules>() {
		@Override
		public NineMenMorrisRules createFromParcel(Parcel in) {
			return new NineMenMorrisRules(in);
		}

		@Override
		public NineMenMorrisRules[] newArray(int size) {
			return new NineMenMorrisRules[size];
		}
	};

	public boolean placeMarker(int to, int player){
		if(gameplan[to] != EMPTY_SPACE){
			return false;
		}else if(gameplan[to] == EMPTY_SPACE && turn == player){
			if(turn == RED_MOVES){
	 ...
	 * Only when all pieces are on the field
	 */
	...
	public boolean win(int color) {
		...
		if (bluemarker == 2 || redmarker == 2 || countMarker < 3)
			return true;
		else
			return false;
	}
	...
	public boolean allMarkersonField(){
	...
	public void writeToParcel(Parcel parcel, int flags) {
		...
		parcel.writeIntArray(gameplan);
		parcel.writeInt(bluemarker);
		parcel.writeInt(redmarker);
		parcel.writeInt(blueTurns);
		parcel.writeInt(redTurns);
